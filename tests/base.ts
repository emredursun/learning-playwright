...
export const authTest = base.extend<AuthFixtures>({
  ...
  adminPage: async ({ browser }, use) => {
    const [context, page] = await getAuthenticatedPage(browser, ".auth/admin.json");
    await use(page);
    await context.close();
  },

  customer1Page: async ({ browser }, use) => {
    const [context, page] = await getAuthenticatedPage(browser, ".auth/customer1.json");
    await use(page);
    await context.close();
  },

  customer2Page: async ({ browser }, use) => {
    const [context, page] = await getAuthenticatedPage(browser, ".auth/customer2.json");
    await use(page);
    await context.close();
  },

  customer3Page: async ({ browser }, use) => {
    const [context, page] = await getAuthenticatedPage(browser, ".auth/customer3.json");
    await use(page);
    await context.close();
  },
});